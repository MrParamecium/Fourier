...
async function generateExplanation(question, bookPages, webSources, options = {}) {
    ...
    const prompt = [
        ...
        language === 'zh' ? '7. 如果需要可视化图示、坐标轴分析、函数图等，不要使用 ASCII 图。请始终返回可直接执行的 Python3 matplotlib 代码块（包含在 ```python 闭包内）。你需要设置 `plt.savefig()` 并将图片保存至 `/tmp/tutor-plot-auto.png` 路径，不要使用 `plt.show()`。重要：代码块必须语法完整、所有字符串必须正确闭合、括号配对，不能截断。只输出用于可视化的独立 python 区块，讲解文字和其他内容保持正常 Markdown 输出。' : '7. Explanations containing coordinate systems, plots or analytical diagrams MUST NOT use ASCII art. You MUST provide runnable Python3 (matplotlib) code blocks wrapped in ```python which save the output figure via `plt.savefig("/tmp/tutor-plot-auto.png", dpi=150)` (do not use plt.show). CRITICAL: the code block MUST be syntactically complete and valid — all strings must be properly closed, all parentheses balanced, no truncation. Deliver the normal textual explanation with this python code block embedded.',
