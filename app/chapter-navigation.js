...
(() => {
  ...
  back.id = 'courseLessonReturn'; back.type = 'button'; back.textContent = '← Lesson overview'; back.hidden = true;
  ...
  function courseCard(title, stats, options = {}) {
    ...
    return `<button type="button" class="course-card" data-course-title="${escapeHtml(title)}"><span class="course-card-copy">${escapeHtml(title)}</span>${progress}<span class="course-card-arrow" aria-hidden="true">↗</span></button>`;
  }
  function chapterSummary(chapter) {
    if (chapter.chapter.startsWith('Chapter 2:')) {
      return 'This chapter builds the time-domain picture of continuous-time systems: first separate internal and external causes, then use impulse response and convolution to predict how a system reacts. It closes by connecting response, stability, and system time constants.';
    }
    return 'This chapter follows the key ideas, models, and tools that connect signals to system behavior. The sequence moves from the core representation to analysis, interpretation, and practical examples.';
  }
  function sectionSummary(title) {
    if (title.startsWith('2.4 ')) {
      return 'This section explains how an external input produces the zero-state response. Start with the convolution integral, build a graphical way to evaluate overlap, then connect individual systems and combine the resulting responses.';
    }
    return 'This section introduces the central idea and examples needed for the lessons on the right. Read the short overview first, then choose the lesson that matches your next step.';
  }
  function showSection(index, section) {
    ...
    shell(index, title, () => {
      paint(`<div class="course-glass course-glass-with-return"><button class="course-return" type="button">← ${escapeHtml(syllabusData[index].chapter)}</button><section><p>SECTION OVERVIEW</p><h1 class="course-title">${escapeHtml(title)}</h1><p class="course-summary">${escapeHtml(sectionSummary(title))}</p></section><section><h2>Lessons</h2><div class="course-list">${[title, ...subs].map(t => courseCard(t, completionStats(t), { showProgress: false })).join('')}</div></section></div>`);
  ...
  function showChapter(index) {
    ...
    shell(index, chapter.chapter, () => {
      paint(`<div class="course-glass"><section><p>CHAPTER OVERVIEW</p><h1 class="course-title">${escapeHtml(chapter.chapter)}</h1><p class="course-summary">${escapeHtml(chapterSummary(chapter))}</p></section><section><h2>Study sequence</h2><div class="course-list">${entries(chapter).map(title => { const sec=chapter.sections.find(x => (typeof x==='string'?x:x.title)===title); const children=typeof sec==='string'?[]:(sec.subsections||[]); return courseCard(title, completionStats(title, children)); }).join('')}</div></section></div>`);
  ...
  function showLesson(index, title) {
    ...
    const lessonBody = isConvolutionLesson
      ? `<section><p>BEFORE YOU BEGIN</p><h1 class="course-title">${escapeHtml(title)}</h1><p class="course-summary">Convolution becomes easier to reason about when you draw the two signals, flip one, and slide it across the other. This section shows how overlap and signed area build one output value at a time.</p><h2>After this overview</h2><ul class="lesson-overview-goals"><li>Identify the interval where two signals overlap.</li><li>Track how shifting changes the product.</li><li>Use the signed area to explain the output.</li></ul></section><section><h2>The idea in one picture</h2><div class="lesson-overview-visual"><svg viewBox="0 0 680 150" role="img" aria-label="Flip, shift, overlap, integrate diagram"><defs><linearGradient id="lessonFlow" x1="0" x2="1"><stop stop-color="#56a4c4"/><stop offset="1" stop-color="#5ab18d"/></linearGradient></defs><g font-family="Inter,system-ui,sans-serif" text-anchor="middle"><g transform="translate(72 58)"><circle r="34" fill="rgba(255,255,255,.5)" stroke="rgba(255,255,255,.8)"/><path d="M-20 10h10v-24h10v24h10" fill="none" stroke="#3c8b88" stroke-width="4" stroke-linejoin="round"/><text y="54" fill="var(--course-ink)" font-size="13" font-weight="700">Flip</text></g><path d="M113 58h58" stroke="url(#lessonFlow)" stroke-width="3"/><path d="m164 50 10 8-10 8" fill="none" stroke="#5a9c87" stroke-width="3"/><g transform="translate(244 58)"><circle r="34" fill="rgba(255,255,255,.5)" stroke="rgba(255,255,255,.8)"/><path d="M-22 10h44M-11 10V-13M-11-13h24" fill="none" stroke="#3c8b88" stroke-width="4" stroke-linecap="round"/><text y="54" fill="var(--course-ink)" font-size="13" font-weight="700">Shift</text></g><path d="M285 58h58" stroke="url(#lessonFlow)" stroke-width="3"/><path d="m336 50 10 8-10 8" fill="none" stroke="#5a9c87" stroke-width="3"/><g transform="translate(416 58)"><circle r="34" fill="rgba(255,255,255,.5)" stroke="rgba(255,255,255,.8)"/><path d="M-22 10h44M-13 10V-7h26v17" fill="none" stroke="#3c8b88" stroke-width="4"/><path d="M-3-7h13" stroke="#d57679" stroke-width="5"/><text y="54" fill="var(--course-ink)" font-size="13" font-weight="700">Overlap</text></g><path d="M457 58h58" stroke="url(#lessonFlow)" stroke-width="3"/><path d="m508 50 10 8-10 8" fill="none" stroke="#5a9c87" stroke-width="3"/><g transform="translate(588 58)"><circle r="34" fill="rgba(255,255,255,.5)" stroke="rgba(255,255,255,.8)"/><path d="M-19 10h38M-13 10V-5l9-10 9 10v15" fill="none" stroke="#3c8b88" stroke-width="4" stroke-linejoin="round"/><text y="54" fill="var(--course-ink)" font-size="13" font-weight="700">Integrate</text></g></g></svg><p class="lesson-overview-caption">One output point comes from the signed product area over the current overlap.</p></div><button type="button" class="course-start" id="courseStartLesson">Start lesson ↗</button></section>`
      : `<section><p>BEFORE YOU BEGIN</p><h1 class="course-title">${escapeHtml(title)}</h1><p class="course-summary">${escapeHtml(intro)}</p></section><section><h2>What you’ll understand</h2><p>${escapeHtml(`The central idea of ${title.replace(/^\S+\s+/, '')}, and how to apply it in the lesson examples.`)}</p><button type="button" class="course-start" id="courseStartLesson">Start lesson ↗</button></section>`;
    shell(index, title, () => {
      paint(`<div class="course-glass course-glass-with-return${isConvolutionLesson ? ' lesson-overview-glass' : ''}"><button class="course-return" type="button">← ${escapeHtml(parent ? parent.title : syllabusData[index].chapter)}</button>${lessonBody}</div>`);
