...
async function initClerk() {
  ...
  if (guestBtnLogin) guestBtnLogin.onclick = () => {
...
function renderUserBadge() {
  if (!settingsUserCard) return;

  if (!currentUser) {
    settingsUserCard.innerHTML = `
      <div class="settings-user-card settings-user-card-empty">
        <div class="settings-user-avatar">?</div>
        <div>
          <div class="settings-user-name">Not signed in</div>
          <div class="settings-user-meta">Use guest mode or sign in to save progress.</div>
        </div>
      </div>
    `;
    return;
    ...
    settingsUserCard.innerHTML = `
      <div class="settings-user-card">
        ...
        <div class="settings-user-body">
          <div class="settings-user-name">Guest</div>
          <div class="settings-user-meta">UID: ${shortUid}</div>
        </div>
        <button class="settings-user-link settings-user-danger" onclick="handleSignOut()" type="button">Exit</button>
      </div>
    `;
  } else {
    const av = currentUser.imageUrl
      ? `<img src="${currentUser.imageUrl}" class="settings-user-avatar-img" />`
      : `<div class="settings-user-avatar">${(currentUser.name[0]||'?').toUpperCase()}</div>`;

    settingsUserCard.innerHTML = `
      <div class="settings-user-card">
        ${av}
        <div class="settings-user-body">
          <div class="settings-user-name">${currentUser.name}</div>
          <div class="settings-user-meta">#${shortUid.toUpperCase()}</div>
        </div>
        <div class="settings-user-actions">
          <button class="settings-user-link" onclick="resetQuiz()" type="button">Preferences</button>
          <button class="settings-user-link settings-user-danger" onclick="handleSignOut()" type="button">Sign out</button>
        </div>
      </div>
    `;
  }
...
function hydrateInteractiveDemos(root) {
  ...
  root.querySelectorAll('.kc-interactive-demo').forEach((node) => {
    ...
    if (!demo || demo.demo_type !== 'matrix_multiplication_conformability') return;
...
function applyLearnChatCollapsedState() {
  ...
  if (learnChatFab) learnChatFab.classList.toggle('hidden', !isLearnChatCollapsed);
...
function applyLearnExplainCollapsedState() {
  ...
  if (learnResizerPanel) learnResizerPanel.classList.toggle('hidden', isLearnExplainCollapsed || isLearnChatCollapsed);
...
function enableFloatingDrag(handle, target, defaults = { right: 28, bottom: 28 }) {
...
async function startLesson() {
  ...
  try {
    ...
    const lessonHtml = isCacheMissLesson
      ...
      : renderHyperknowLinks(data.webSources || []) + markdownToHtml(data.lesson || 'No explanation available.');
...
function _setLearnMode(mode) {
  ...
  if (learnChatColPanel) {
    learnChatColPanel.style.display = (_learnLayoutMode === 'overview' || isLearnChatCollapsed) ? 'none' : '';
  }
  if (learnResizerPanel) {
    learnResizerPanel.style.display = (_learnLayoutMode === 'overview' || isLearnChatCollapsed) ? 'none' : '';
  }
  if (learnExplainColEl) {
    learnExplainColEl.style.flex = _learnLayoutMode === 'overview' ? '1' : '5.5';
...
function updateRecentConversationsUI() {
  ...
  sessions.forEach((session) => {
    ...
    if (truncated.length > 24) truncated = truncated.slice(0, 24) + '...';
